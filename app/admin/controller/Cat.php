...
class Cat extends Common
{
    ...
    public function del(){
      $id=input('id/d');
      $check_del=Db::table('cat')->where('cat_id',$id)->update(['delete_time'=>time()]);
      if($check_del){
             $cat_list=Db::table('cat')->where('cat_id',$id)->column('cat_id');
             $ids=implode(',',$cat_list);
             Db::table('cat')->where('cat_id','in',$ids)->update(['delete_time'=>time()]);
    ...
    public function delAll(){
        ...
        if(!empty($id)){
          $date['delete_time']=time();
            $uploads=Db::table('cat')->where('cat_id','in',$id)->update($date);
            if($uploads){
                Db::table('cat')->where('parent_id','in',$id)->update($date);
                echo 1;
            }else{
                echo 3;
            }
